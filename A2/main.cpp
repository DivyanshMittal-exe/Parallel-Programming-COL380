...
int main(int argc, char** argv)
{
    ...
    int partial_sums_recv[world_size];

    partial_sums_recv[0] = length_recv[0];

    for(int i=1; i<world_size; i++)
    {
        partial_sums_recv[i] = partial_sums_recv[i-1]+length_recv[i];
    }
    ...
    for(int i=1; i<world_size; i++)
    {
        displ[i] = 2*partial_sums_recv[i-1];
    }
    

    int* recv_queries = (int*)malloc(sizeof(int)*partial_sums_recv[world_size-1]);
