...
int main(int argc, char** argv)
{
    ...
    MPI_File input_data;
    MPI_File_open(MPI_COMM_WORLD, "0.gra", MPI_MODE_RDONLY, MPI_INFO_NULL, &input_data);
    // MPI_File_open(MPI_COMM_WORLD, "/tmp/COL380/A2/test0/test-input-0.dat", MPI_MODE_RDONLY, MPI_INFO_NULL, &input_data);

    MPI_File meta_data;
    MPI_File_open(MPI_COMM_WORLD, "0.dat", MPI_MODE_RDONLY, MPI_INFO_NULL, &meta_data);
    ...
    //                     if(neighbour_data[id_index][coord2+4] == 1 && neighbour_data[id_index][coord1+4] == 1)
    //                     {
    ...
    //                 if(neighbour_data[id_index][coord_id_node2+4] == 1 && neighbour_data[id_index][coord_id_node1+4] == 1)
    //                 {
    ...
    //             {
    //                 if(neighbour_data[i][5*j+1]<=0 && neighbour_data[i][5*j+4] == 1)
    //                 {
    //                     edge_deleted = 1;
    //                     neighbour_data[i][5*j+4] = 0;
    //                     neighbour_data[i][5*j+3] = K-1;
    //                 }
    //             }
    //         }

    //         MPI_Allreduce(&edge_deleted, &all_edge_deleted, 1, MPI_CXX_BOOL, MPI_LOR, MPI_COMM_WORLD);
