...
struct tau_hat_e_struct{
    ...
    tau_hat_e_struct(int e,int k,int s){
        edge_second = e;
        k_val = k;
        settled = s;
    }

    bool operator<(const tau_hat_e_struct& other) const {
        return edge_second < other.edge_second;
    }
...
//    map<pair<int,int>,pair<int,int>> tau_hat_e;
    ...
    vector<vector<tau_hat_e_struct>> tau_hat_e(graph.size());



    for(int i = 0; i < graph.size(); i++){
        tau_hat_e[i].resize(graph[i].neighbours.size());
        for (int j = 0; j < graph[i].neighbours.size(); ++j) {
            tau_hat_e[i][j] = tau_hat_e_struct(graph[i].neighbours[j].node_val,0,0);
        }
    }
...
//                auto it = lower_bound(tau_hat_e[n_node_1.node_val/size].begin(), tau_hat_e[n_node_1.node_val/size].end(), {n_val_temp, 0, 0});

                auto it = lower_bound(tau_hat_e[n_node_1.node_val / size].begin(),
                                            tau_hat_e[n_node_1.node_val / size].end(),
                                            tau_hat_e_struct{n_val_temp,0,0}
                                           );
...
//        }
    ...
    for(int i = 0; i < n; i++){
        std::sort(tau_hat_tri[i].begin(), tau_hat_tri[i].end());
    }
...
//        int k_min_loc = INT_MAX;
        ...
        int my_edges = 0;
        for(int i = 0; i < tau_hat_e.size(); i++) {
...
//                k_min_loc = min(k_min_loc,a.first);
        ...
        while (true) {
            ...
            for(int ii = 0; ii < tau_hat_e.size(); ii++){

                for (auto const &pr: tau_hat_e[ii]) {

                    auto edge_first = graph[ii].node_val;
...
//                                bloom_filter[h2(pr.edge_second,neighb.node_val) % bloom_size] &&
                            ...
                            auto under_consider = lower_bound(tau_hat_e[ii].begin(), tau_hat_e[ii].end(), tau_hat_e_struct(neighb.node_val, 0, 0));


//                            auto &under_consider = tau_hat_e[{edge_first, neighb.node_val}];

                        if(!under_consider->settled){
                            //                     Case where both are min
                            if (under_consider->k_val < k - 2) {
                                // Only 1 of them sends
                                if (pr.edge_second < neighb.node_val) {
                                    decrement_queries[pr.edge_second % size].push_back({pr.edge_second, neighb.node_val});
                                    decrement_queries[neighb.node_val % size].push_back(
                                            {neighb.node_val, pr.edge_second});
...
//                cout << rank << " " << q.u << " " << q.v << endl;

                auto edge_to_pot_dec = lower_bound(tau_hat_e[q.u/size].begin(), tau_hat_e[q.u/size].end(), tau_hat_e_struct(q.v, 0, 0));

                if (!edge_to_pot_dec->settled) {
                    edge_to_pot_dec->k_val--;
                }
            }
    ...
    if(verbose == 0){
        ...
        for(int i = 0; i < tau_hat_e.size(); i++){
            for(const auto &tau_it: tau_hat_e[i]){
                max_k_loc = max(max_k_loc,tau_it.k_val);
            }
...
//            auto const &tau_val = tau_it.second;
                ...
                if (graph[ii].node_val < pr.edge_second) {
                    auto new_tau_edge = tau_edge{graph[ii].node_val, pr.edge_second, pr.k_val};
